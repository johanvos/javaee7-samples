		...
		<table width="100%">
			...
			<tbody>
				<c:forEach var="message" items="${messages}" varStatus="stat">
					<tr>
						<td valign="top">${message.author.screenName} [${message.author.score}]</td>
						<td valign="top">
							<p><b>${message.title} [${message.score}]</b></p>
							<p>${message.content}</p>
						</td>
						<td valign="top">
						...
						<td>
							...
							<a href="comment.jsp?message=${message.id}">comment</a><br/>
							<a href="rest/vote/content/${message.id}">vote</a>
						</td>
					</tr>
					<c:forEach var="comment" items="${comments[message.id]}">
						<tr>
							<td valign="top"><small>${comment.author.screenName} [${comment.author.score}]</small></td>
							<td valign="top">
